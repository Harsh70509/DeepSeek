...
const PromptBox = ({setIsLoading, isLoading}) => {
    ...
    const sendPrompt = async (e) => {
        ...
        try {
            e.preventDefault();
            if(!user) return toast.error("Login to send a prompt");
    ...
    className={`w-full ${selectedChat?.messages.length > 0 ? "max-w-3xl" : "max-w-2xl"} bg-[#404045] p-4 rounded-3xl mt-4 transition-all`}>
        ...
        <div className='flex items-center justify-between text-sm'>
            <div className='flex items-center gap-2'>
                <p className='flex items-center gap-2 text-xs border text-gray-300/40 px-2 py-1 rounded-full cursor-pointer hover:bg-gray-500/20 transition'>
